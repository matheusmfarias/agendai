...
export function WhatsAppSettingsPanel({ allowed }: { allowed: boolean }) {
  ...
  return (
    <Card className="border-border/70 bg-card/95 shadow-sm"><CardContent className="space-y-6 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">WhatsApp</h2><p className="text-sm text-muted-foreground">Conecte um número para enviar confirmações transacionais de agendamento.</p><p className="mt-1 text-xs text-muted-foreground">Beta baseado em WhatsApp Web: a conexão pode exigir uma nova leitura do QR Code.</p></div>{connection ? <Badge variant={connection.status === "CONNECTED" ? "success" : "outline"}>{STATUS_LABEL[connection.status]}</Badge> : null}</div>
