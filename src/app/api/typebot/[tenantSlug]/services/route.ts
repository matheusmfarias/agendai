...
import { buildServicesText, getTypebotServices, getTypebotTenant, validateTypebotTenant } from "@/features/typebot/typebot-service";
...
export async function GET(
  ...
  const guard = await guardTypebotEndpoint(_request, tenantSlug, "services");
  ...
  return typebotOk({
    services,
    text: buildServicesText(services),
  });
}
